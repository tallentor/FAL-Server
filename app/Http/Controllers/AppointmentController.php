...
class AppointmentController extends Controller
{
    ...
    public function storeWada(Request $request)
    {
        ...
        $hasProfile = LawyerProfile::where('user_id', $request->lawyer_id)->exists();
        if (! $hasProfile) {
    ...
    public function storeAppointment(Request $request)
{
    ...
    // Appointment approve
    public function approveAppointment12(Request $request, $appointmentId)
{
    try {
        $lawyer = Auth::user();
        ...
        // Find appointment assigned to this lawyer
        $appointment = Appointment::where('lawyer_id', $lawyer->id)
            ->where('id', $appointmentId)
            ->first();

        if (!$appointment) {
            return response()->json(['message' => 'Appointment not found or not assigned to this lawyer'], 404);
        }
        ...
        // Set current date & time automatically
         ...
         $meetingTime = $appointment->appointment_time;

        // Create Zoom meeting
        $zoomController = new \App\Http\Controllers\ZoomController();
        $zoomLink = $zoomController->createZoomMeeting(Carbon::parse("{$meetingDate} {$meetingTime}"), $lawyer->name);
    ...
    try {
        ...
        if ($admin) {
            AppointmentNotification::create([
                ...
                'date' => $appointment->appointment_date,
                'time' => $appointment->appointment_time,
                'zoom_link' => $joinUrl, // ✅ Fixed: only string
                'status' => 'unread',
            ]);
        }
